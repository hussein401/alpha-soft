        ...
        <div class="container">
            ...
            <div class="flex flex-wrap justify-center gap-8 md:gap-12 lg:gap-16">
                ...
                <a href="{{ route('laptops') }}" class="group flex flex-col items-center gap-4 transition-all">
                    <div class="w-28 h-28 md:w-36 md:h-36 rounded-full flex items-center justify-center transition-all duration-500 shadow-xl {{ !request()->has('category') ? 'bg-primary ring-4 ring-primary ring-offset-4 ring-offset-white scale-110' : 'bg-slate-100 border border-slate-200 hover:border-primary/50 hover:scale-105' }}">
                        <i class="fa-solid fa-border-all text-3xl md:text-4xl {{ !request()->has('category') ? 'text-white' : 'text-primary' }}"></i>
                    </div>
                    <span class="text-xs md:text-sm font-black uppercase tracking-widest {{ !request()->has('category') ? 'text-primary' : 'text-slate-400 group-hover:text-primary' }}">All Brands</span>
                </a>
                ...
                @foreach($categories as $cat)
                    ...
                    <a href="{{ route('laptops', ['category' => $cat->slug]) }}" class="group flex flex-col items-center gap-4 transition-all">
                        <div class="w-28 h-28 md:w-36 md:h-36 rounded-full overflow-hidden bg-[#FEF0E0] flex items-center justify-center p-4 transition-all duration-500 shadow-xl {{ request('category') == $cat->slug ? 'ring-4 ring-primary ring-offset-4 ring-offset-white scale-110' : 'hover:scale-105' }}">
                        ...
                        <span class="text-xs md:text-sm font-black uppercase tracking-widest {{ request('category') == $cat->slug ? 'text-primary' : 'text-slate-400 group-hover:text-primary' }}">{{ $cat->name }}</span>
                    </a>
                @endforeach
            </div>
        </div>
