                ...
                <div class="bg-slate-900/60 backdrop-blur-2xl border border-slate-800 rounded-[2.5rem] p-6 md:p-8 shadow-[0_30px_60px_rgba(0,0,0,0.4)] relative overflow-hidden">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 relative z-10">
                        ...
                        <a href="{{ route('laptops') }}" 
                           class="flex flex-col items-center justify-center p-6 rounded-3xl border transition-all duration-300 {{ !request()->has('category') ? 'bg-primary border-primary text-white shadow-[0_10px_20px_rgba(6,182,212,0.3)]' : 'bg-slate-800/40 border-slate-800 text-gray-400 hover:border-primary/40 hover:text-white group' }}">
                            <div class="w-10 h-10 rounded-xl flex items-center justify-center mb-3 {{ !request()->has('category') ? 'bg-white/20' : 'bg-slate-700 group-hover:bg-primary/20' }}">
                        ...
                        @foreach($categories as $cat)
                            <a href="{{ route('laptops', ['category' => $cat->slug]) }}" 
                               class="flex flex-col items-center justify-center p-6 rounded-3xl border transition-all duration-300 {{ request('category') == $cat->slug ? 'bg-primary border-primary text-white shadow-[0_10px_20px_rgba(6,182,212,0.3)]' : 'bg-slate-800/40 border-slate-800 text-gray-400 hover:border-primary/40 hover:text-white group' }}">
                                @php
                                    $icon = 'fa-laptop';
                                ...
                                @endphp
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center mb-3 {{ request('category') == $cat->slug ? 'bg-white/20' : 'bg-slate-700 group-hover:bg-primary/20' }}">
                                    <i class="fa-solid {{ $icon }} text-sm uppercase"></i>
                                </div>
                                <span class="font-bold text-xs uppercase tracking-widest">{{ $cat->name }}</span>
                            </a>
                        @endforeach
                    </div>
                </div>
